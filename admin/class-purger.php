...
abstract class Purger
{
     ...
     */
    final public function check_and_truncate_log_file()
    {
        ...
        if ($file_size > $max_size_allowed) {
            ...
            $file_content = empty($file_content) ? '' : mb_strstr($file_content, "\n");
     ...
     * @return string
     */
    final public function check_http_connection()
    {
     ...
     * @return bool|void
     */
    final public function log($msg, $level = 'INFO')
    {
     ...
     */
    final public function purge_image_on_edit($attachment_id)
    {
     ...
     * @return bool
     */
    final public function purge_on_check_ajax_referer($action)
    {
     ...
     * @return bool|void
     */
    final public function purge_on_post_moved_to_trash($new_status, $old_status, $post)
    {
        ...
        if ($new_status === 'trash') {
            ...
            if ((int) $nginx_helper_admin->options['purge_homepage_on_del'] === 1) {
                $this->_purge_homepage();
            }
     ...
     * @return bool
     */
    final public function purge_on_term_taxonomy_edited($term_id, $tt_id, $taxon)
    {
        ...
        if ($current_filter === 'edit_term' && ! is_wp_error($term) && ! empty($term)) {
        ...
        } elseif ($current_filter === 'delete_term') {
     ...
     */
    final public function purge_post($post_id)
    {
        ...
        if ((int) $nginx_helper_admin->options['purge_homepage_on_edit'] === 1) {
            $this->_purge_homepage();
        }
     ...
     */
    final public function purge_post_on_comment($comment_id, $comment)
    {
     ...
     * @param  object  $comment  Comment data.
     */
    final public function purge_post_on_comment_change($newstatus, $oldstatus, $comment)
    {
        ...
        switch ($newstatus) {
            ...
            case 'approved':
                if ((int) $nginx_helper_admin->options['purge_page_on_new_comment'] === 1) {
            ...
            case 'unapproved':
            case 'trash':
                if ($oldstatus === 'approved' && (int) $nginx_helper_admin->options['purge_page_on_deleted_comment'] === 1) {
    ...
    /**
     * Purge the canonical URL of a post and all variants registered via
     * the rt_nginx_helper_purge_urls filter.
     *
     * Convenience helper for callers that don't go through the full
     * purge_post() flow but still want alternate representations
     * (.md, .amp, .json, …) to be invalidated alongside the HTML page.
     *
     * @param  int  $post_id  Post id.
     * @return void
     */
    final public function purge_post_url_with_variants($post_id)
    {
     ...
     * @return bool
     */
    final public function purge_them_all()
    {
     ...
     * @return void
     */
    final public function unlink_recursive($dir, $delete_root_too)
    {
    ...
    protected function delete_cache_file_for($url)
    {
        ...
        $cache_path = (mb_substr($cache_path, -1) === '/') ? $cache_path : $cache_path.'/';

        // Set path to cached file.
        $cached_file = $cache_path.mb_substr($hash, -1).'/'.mb_substr($hash, -3, 2).'/'.$hash;
    ...
    private function _purge_all_customtaxa()
    {
        ...
        if (! empty($custom_taxonomies)) {
            ...
            foreach ($custom_taxonomies as $taxon) {
                ...
                if (! in_array($taxon, ['category', 'post_tag', 'link_category'], true)) {
    ...
    {
        $extra_urls = apply_filters('rt_nginx_helper_purge_urls', [], $post_id);
